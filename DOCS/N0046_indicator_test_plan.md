...
typedef enum {
    ...
    TEST_PHASE_WHITE_BLINK_SLOW,    // 白灯慢闪（亮 1s + 灭 1s）
    TEST_PHASE_RED_BLINK_FAST,      // 红灯快闪（亮 500ms + 灭 500ms）
...
| 3 | T=9s | 白灯 A15 | 慢闪（亮1s+灭1s） | 白灯以约 2s 周期规律闪烁，亮灭时间相等 | 肉眼可清晰分辨 1 秒亮、1 秒灭的节奏 |
| 4 | T=12s | 红灯 B23 | 快闪（亮500ms+灭500ms） | 红灯以约 1s 周期闪烁，频率高于阶段 3 | 可看出红灯闪烁节奏快于慢闪，且亮灭各约 500ms |
