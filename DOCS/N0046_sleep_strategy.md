...
| 以 04122-13A 为平台基线 | 参考其 GPIO/RTC/PA2 的睡前准备和唤醒恢复；启用 `HAL_SLEEP=1` 复用 WCH 的 TMOS 睡眠执行链路，在其上构建本项目的 LPM 事件状态机 |
...
| HAL_SLEEP 开关 | 04122-13A 使用 `HAL_SLEEP=1`；本地代码当前为 `HAL_SLEEP=0` | **必须将 `CONFIG.h` 中 `HAL_SLEEP` 改为 `TRUE`**，否则 RTC 唤醒初始化和 BLE 低功耗回调均不生效 |
...
| 矩阵方向 | N0046 是 `COL2ROW`，与 04122-13A 一致 | 矩阵 prepare 方向相同：COL 拉低输出，ROW 配下降沿中断 |
| `PM_STANDBY_WITH_RAM` | 当前本地路径不完整，`LowPower_Shutdown(RB_PWR_RAM2K)` 被注释 | Deep 首版不走该路径；采用 Option A（HAL_SLEEP=1），由 TMOS idleCB 驱动实际睡眠 |

### 3.3 CH584 底层强约束

| 约束 | 要求 |
|---|---|
| HAL_SLEEP 必须开启 | `project/ch584m/platforms_HAL/include/CONFIG.h` 中 `HAL_SLEEP` 必须为 `TRUE`；关闭时 `HAL_SleepInit()`、`cfg.idleCB` 注册、`CH58x_LowPower()` 核心逻辑全部失效 |
| RTC 初始化由 HAL_Init 完成 | `HAL_SLEEP=1` 后，`HAL_Init()` 自动调用 `HAL_SleepInit()` 配置 `RB_SLP_RTC_WAKE`、`RB_RTC_TRIG_EN`、`RTC_IRQn`；上层无需重复初始化 |
| RTC 寄存器写入 | 必须参考 WCH `RTC_SetTignTime()` 的安全访问方式（`sys_safe_access_enable/disable`），不能裸写触发寄存器 |
| RTC 中断处理 | 必须在中断里清 RTC 标志并锁存唤醒原因（`RTCTigFlag = 1`） |
| GPIO 唤醒顺序 | 必须先配置 GPIO，再清中断标志，再开 IRQ，最后使能 `RB_SLP_GPIO_WAKE` |
| 唤醒后恢复 | 必须恢复矩阵、PA2、触控等实际中断配置，不仅是恢复软件标志；PA2 恢复必须按"设输入上拉 → 配下降沿中断 → 清标志"顺序执行 |
| 睡眠参数验证 | `LowPower_Sleep(...)` 的位组合由 `CH58x_LowPower()` 固定，以 04122/WCH 为基线，并经过 N0046 实机验证 |
...
| 12 | `system_service` | 确认 prepare 全部完成，状态切到 `LPM_STATE_IDLE_SLEEP`；实际 MCU 睡眠由 TMOS `idleCB`（`CH58x_LowPower()`）在下一个 BLE 空闲窗口自动触发，无需显式调用 `system_hal_enter_sleep()` | MCU 在下一 BLE 空闲窗口进入 Idle Sleep |
...
| 13 | `system_service` | 确认 prepare 全部完成，状态切到 `LPM_STATE_DEEP_SLEEP`；实际 MCU 睡眠由 TMOS `idleCB`（`CH58x_LowPower()`）自动触发，无需显式调用 `system_hal_enter_sleep()` | MCU 在下一 BLE 空闲窗口进入 Deep Sleep |
...
采用 Option A（HAL_SLEEP=1）后，RTC 唤醒由 TMOS `CH58x_LowPower()` 自动管理；GPIO 唤醒由 `input_service` 在 prepare 阶段完成外设配置后，统一通过 `PWR_PeriphWakeUpCfg(ENABLE, RB_SLP_GPIO_WAKE, Long_Delay)` 开启。

| 抽象唤醒源 | CH584 实际落点 | 说明 |
|---|---|---|
| `SYSTEM_WAKEUP_GPIO` | `RB_SLP_GPIO_WAKE` | 用于矩阵 ROW、触控 INT、PA2 等所有 GPIO 唤醒；由 `input_service` 在 prepare 阶段配置 |
| `SYSTEM_WAKEUP_TIMER` | `RB_SLP_RTC_WAKE` + `RTC_SetTignTime()` | Option A 下由 TMOS `CH58x_LowPower()` 自动计算和配置，上层无需直接操作 |
| `SYSTEM_WAKEUP_RTC` | `RB_SLP_RTC_WAKE` | 与 `SYSTEM_WAKEUP_TIMER` 的硬件落点一致，由 TMOS 管理 |
| `SYSTEM_WAKEUP_USB` | `RB_SLP_USB_WAKE` | 仅在 USB 模式下启用；N0046 当前 `USB_ENABLE=FALSE`，Deep 中纳入 GPIO 低功耗处理 |
| `SYSTEM_WAKEUP_KEYBOARD` | 不应直接映射为独立硬件位 | 键盘最终属于 GPIO 唤醒（ROW 引脚中断） |
| `SYSTEM_WAKEUP_BLE` | 不应直接映射为独立硬件位 | BLE 低功耗是协议状态，不是 CH584 独立唤醒位 |

### 8.2 RTC 设计要求

采用 Option A（HAL_SLEEP=1）后，RTC 初始化和触发配置由 WCH HAL 自动完成，无需上层手动操作。

| 要点 | 要求 |
|---|---|
| 初始化 | `HAL_SLEEP=1` 时 `HAL_Init()` 自动调用 `HAL_SleepInit()`，配置 `RB_SLP_RTC_WAKE`、`RB_RTC_TRIG_EN`、`RTC_IRQn`；上层禁止重复初始化 |
| 触发时间配置 | 由 `CH58x_LowPower(time)` 内部调用 `RTC_SetTignTime()`，计算提前唤醒量后自动写入 |
| 安全访问 | 写 RTC 触发寄存器时必须使用 CH584 安全访问流程（`sys_safe_access_enable/disable`） |
| 中断清标志 | `RTC_IRQHandler()` 清 RTC 标志并置 `RTCTigFlag = 1`，`CH58x_LowPower()` 检查此标志防止超时误睡 |
| 无效窗口保护 | `CH58x_LowPower()` 已内置：若 `time_sleep < SLEEP_RTC_MIN_TIME` 或 `> SLEEP_RTC_MAX_TIME` 则跳过本次睡眠 |

### 8.3 Sleep 参数要求

采用 Option A（HAL_SLEEP=1）后，睡眠由 TMOS `CH58x_LowPower()` 统一执行，参数已固定为经过 WCH 验证的组合，不通过 `system_hal_enter_sleep()` 传入。

| 要点 | 要求 |
|---|---|
| 参数固定路径 | `CH58x_LowPower()` → `LowPower_Sleep(RB_PWR_RAM32K \| RB_PWR_RAM96K \| RB_PWR_EXTEND \| RB_XT_PRE_EN)` |
| `RB_XT_PRE_EN` 必须保留 | 保证睡眠期间晶振持续预热，BLE 唤醒后时钟恢复快，防止连接丢失 |
| 唤醒后 HSE 恢复 | `CH58x_LowPower()` 返回后自动执行 `HSECFG_Current(HSE_RCur_100)`，无需 HAL 额外处理 |
| 首版禁止改动参数组 | 不得删除 `RB_XT_PRE_EN` 或修改 RAM 保留位，以 04122/WCH 基线为准 |
| 实机校验 | 需在 N0046 上验证 Idle/Deep 均能稳定进入和退出，BLE 连接/时钟恢复正常 |

### 8.4 HAL 的边界

采用 Option A 后，HAL 边界进一步收窄：`LowPower_Sleep()` 和 RTC 操作均由 TMOS HAL（`CH58x_LowPower()`）负责，`_system_hal.c` 不再直接调用 `LowPower_Sleep()`。

| 允许做的事 | 不允许做的事 |
|---|---|
| 配置 GPIO 唤醒源（`PWR_PeriphWakeUpCfg`）、锁存唤醒原因、投递 OSAL 唤醒事件 | 直接调用 `LowPower_Sleep()`（由 TMOS idleCB 负责）、调背光、关指示灯、断 BLE、停键盘扫描、触发业务重连 |
...
| `drivers/input/keyboard/matrix.c/.h` | 增加 `matrix_prepare_wakeup()`、`matrix_resume_from_sleep()`、`matrix_scan_once_after_wakeup()` | N0046 为 `COL2ROW`：COL 拉低输出，ROW 配下降沿中断，与 04122-13A 一致 |
| `hal/system_hal.h` | 明确 `SYSTEM_WAKEUP_TIMER` 的平台语义；增加唤醒原因查询接口 | 避免把软件定时器误当成硬件唤醒 |
| `hal/platforms/ch584/_system_hal.c` | 移除直接调用 `LowPower_Sleep()` 的路径（由 TMOS HAL 接管）；保留 GPIO 唤醒源配置接口；补充唤醒原因锁存接口 | Option A 下 HAL 不再主导睡眠执行，仅负责唤醒源配置 |
| `application/service/wireless_callbacks.c` | 去掉直接进入睡眠的路径，统一改为投递 OSAL 事件 | 保持所有关键动作都走 OSAL |
| `keyboards/n0046/config.h` | 增加 Idle/Deep 超时和行为配置项；将 `DIODE_DIRECTION` 确认为 `COL2ROW` | 方便后续调参与产品化；修正矩阵方向配置 |
| `project/ch584m/platforms_HAL/include/CONFIG.h` | 将 `HAL_SLEEP` 改为 `TRUE` | 启用 TMOS RTC 睡眠链路和 BLE idleCB，这是整套睡眠方案的前提条件 |
...
| 矩阵方向适配 | `COL2ROW` 下 COL 拉低、ROW 配中断是否正确，是否不会出现睡死、假唤醒、首键丢失 |
