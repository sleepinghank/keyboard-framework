...
**验证标准：**
- 零警告零错误
- `grep -r "indicator_set" middleware/` 无结果
- `grep -r "indicator_set" keyboards/` 无结果
...
```c
/* stub：替换 indicator_set，记录调用参数 */
static uint8_t            stub_led_id;
static const ind_effect_t *stub_effect;
static uint8_t            stub_call_count;
...
/* 测试用例 */
void test_bt_pairing_routes_to_white_led(void) {
    stub_call_count = 0;
    output_service_request_indicator(IND_REQ_BT_PAIRING, 1);
    simulate_osal_event(OUTPUT_INDICATOR_EVT);

    TEST_ASSERT_EQUAL(1, stub_call_count);
    TEST_ASSERT_EQUAL(LED_WHITE, stub_led_id);
    TEST_ASSERT_EQUAL_PTR(&IND_BLINK_SLOW, stub_effect);
}

void test_caps_overwrites_bt_state(void) {
    output_service_request_indicator(IND_REQ_BT_CONNECTED, 1);
...
void test_unknown_type_ignored(void) {
    stub_call_count = 0;
    output_service_request_indicator((ind_req_type_t)0xFF, 0);
    TEST_ASSERT_EQUAL(0, stub_call_count);
}

void test_lpm_resume_replays_pending(void) {
    output_service_request_indicator(IND_REQ_LOW_BATTERY, 0);
    simulate_osal_event(OUTPUT_INDICATOR_EVT);  /* 正常处理，清 dirty */

    stub_call_count = 0;
    simulate_osal_event(OUTPUT_LPM_RESUME_EVT); /* resume 重放 */
    TEST_ASSERT_EQUAL(1, stub_call_count);
    TEST_ASSERT_EQUAL(LED_RED, stub_led_id);
}
```
