...
uint8_t AES_Keyinitialization(uint8_t *addr,uint32_t productModelCode, uint32_t chipModelCode)
{
  ...
  for(z = 0; z < MAX_AES_KEY_SIZE; z++)
  {
    for (i = 0; i < 16; i++)
    {
      ...
      case 13: key = (AES_DeviceAddress[1] - AES_DeviceAddress[3])  ^ original_key[z][i]; break;  
      case 14: key = (AES_DeviceAddress[2] + AES_DeviceAddress[4])  ^ original_key[z][i]; break;  
      case 15: key = (AES_DeviceAddress[3] - AES_DeviceAddress[5])  ^ original_key[z][i]; break;  
      default:
        break;
      }
  ...
  for(z = 0; z < MAX_AES_KEY_SIZE; z++)
  {
    ...
    for (i = 0; i < 16; i++)
    {
      DEBUG_logs("%02x ",aes_key[z][i]);
    }
    DEBUG_logs("} \r\nIV[%d]{",z);
    for (i = 0; i < 16; i++)
    {
      DEBUG_logs("%02x ",aes_iv[z][i]);
    }
    DEBUG_logs("}");
  }
#endif
