...
**并行子任务管理**：使用 `Agent` 工具派遣子任务。`Agent` 是同步调用——**并行的唯一方式是在同一条回复（response）中同时发出所有 Agent tool call**，系统会并发执行它们，每个调用直接返回结果，无需轮询。如果先发一个 Agent、等它完成、再发下一个，则是串行执行。如结果不满意，重新派遣新的 `Agent` 即可。
...
| 先发一个 Agent 等结果，再发下一个 | 在同一条回复中同时发出所有 Agent 调用（并发 tool call）|
...
所有问题回答完毕后，在**同一条回复**中同时发起 3 个 `Agent` tool call（参考 `references/agent-roles.md` 中「需求评审团」）：
...
每个 Agent 输出限制 200 字以内。

⚠️ **并行要求**：必须在**同一条回复**中同时发出全部 3 个 Agent 调用，不要等待任何一个完成后再发下一个——否则退化为串行。如结果不满意，重新派遣新 Agent 即可。
...
⚠️ **并行要求**：必须在**同一条回复**中同时发出全部 5 个 Agent 调用，不要等待任何一个完成后再发下一个——否则退化为串行。如某个结果不满意，单独重新派遣新 Agent，无需重跑全部。
