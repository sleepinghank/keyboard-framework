...
## 步骤 2 — 需求澄清与 Agent Team 评审

声明：`【步骤 2/9 — 需求澄清与 Agent Team 评审】`
...
## 步骤 3 — 代码现状分析（并行 Agent Team）

声明：`【步骤 3/9 — 代码现状分析（并行分析中...）】`

### 并行执行策略

根据步骤 1 识别的相关模块，将分析任务拆分给多个 Agent Team 并行执行：

**Agent Team 分配规则：**
- 每个 Agent Team 负责一个独立模块/子系统
- 模块数 ≤ 3：每模块一个 Agent Team
- 模块数 > 3：按功能层次合并，保持 Agent Team 数量 ≤ 5
...
**主线程汇总规则：**
- 收集所有 Agent Team 的结论输出
- 不保留 Agent Team 的分析过程，只保留结论
- 生成跨模块的数据流图（文本形式）
