...
// HID Report characteristic，所有报告，键盘，鼠标，触摸板都使用这个
...
// 物理尺寸.3*100.4mm
// x y 轴物理最大值
#define IOS_HID_TOUCHPAD_MAX_PHYSICAL_X  1016//1610//1173 // 显示值  设备代表值的边界
#define IOS_HID_TOUCHPAD_MAX_PHYSICAL_Y  555//822//600 
// 逻辑最大值
#define IOS_HID_TOUCHPAD_MAX_LOGICAL_X 	(1728-1)//2729//1990//0x578  // 实际值 1293   测量值 读数 设备返回数值的边界
#define IOS_HID_TOUCHPAD_MAX_LOGICAL_Y  (944-1)//1394//1017//0x320  // 800

#define FINGER_IOS(FINGER_NUMBER) \
	...
	0xA1, 0x00 , /*  COLLECTION (PHYSICAL)  */ \
		...
		0x46, IOS_HID_TOUCHPAD_MAX_PHYSICAL_X & 0xff, \
			IOS_HID_TOUCHPAD_MAX_PHYSICAL_X >> 8, /*  PHYSICAL MAXIMUM (921)  */ \
		0x26, IOS_HID_TOUCHPAD_MAX_LOGICAL_X & 0xff, \
			IOS_HID_TOUCHPAD_MAX_LOGICAL_X >> 8, /*  LOGICAL MAXIMUM (1105)  */ \
		0x15, 0x00,/* LOGICAL_MINIMUM (0)*/ \
		0x75, 0x0c, /*  REPORT SIZE (12)  */ \
		0x55, 0x0e,/* UNIT_EXPONENT (-2) */ \
		0x65, 0x11,/* UNIT(SI Length (cm)) 单位*/ \
		0x09, 0x30 , /*  USAGE (X)  */ \
		0x81, 0x02 , /*  INPUT (Data,Var,Abs)  */ \
		0x35, 0x00,/* PHYSICAL_MINIMUM (0) */ \
		0x46, IOS_HID_TOUCHPAD_MAX_PHYSICAL_Y & 0xff, \
			IOS_HID_TOUCHPAD_MAX_PHYSICAL_Y >> 8, /*  PHYSICAL MAXIMUM (506)  */ \
		0x15, 0x00,/* LOGICAL_MINIMUM (0)*/ \
		0x26, IOS_HID_TOUCHPAD_MAX_LOGICAL_Y & 0xff, \
			IOS_HID_TOUCHPAD_MAX_LOGICAL_Y >> 8, /*  LOGICAL MAXIMUM (607)  */ \
		0x09, 0x31 , /*  USAGE (Y)  */ \
		0x81, 0x02 , /*  INPUT (Data,Var,Abs)  */ \
...
#if TOUCH
//声明触摸板特征变量
// HID Report characteristic, Touchpad input
static uint8_t       hidReportTouchpadInProps = GATT_PROP_READ | GATT_PROP_NOTIFY;
static uint8_t       hidReportTouchpadIn;
static gattCharCfg_t hidReportTouchpadInClientCharCfg[GATT_MAX_NUM_CONN];

// HID Report Reference characteristic descriptor, Touchpad input
static uint8_t hidReportTouchpadRefIn[HID_REPORT_REF_LEN] =
    {HID_RPT_ID_TOUCHPAD_IN, HID_REPORT_TYPE_INPUT};


// 增加一个触控板FEATURE，用于报告表面触点，需要实现GATT 读写模式
// HID Report characteristic, Touchpad Feature
static uint8_t       hidReportTouchpadFeatureProps = GATT_PROP_READ | GATT_PROP_WRITE;
static uint8_t      hidReportTouchpadFeature[2];

static uint8_t       hidReportTouchpadFeatureRef[HID_REPORT_REF_LEN] =
    {HID_RPT_ID_TOUCHPAD_FEATURE, HID_REPORT_TYPE_FEATURE};
...
#if TOUCH
    // HID Report characteristic, Touchpad input declaration 添加GATT属性表
    {
        {ATT_BT_UUID_SIZE, characterUUID},
        GATT_PERMIT_READ,
        0,
        &hidReportTouchpadInProps},

        // HID Report characteristic, Touchpad input
        {
            {ATT_BT_UUID_SIZE, hidReportUUID},  // 使用相同的UUID (0x2A4D)
            GATT_PERMIT_ENCRYPT_READ,
            0,
            &hidReportTouchpadIn},
        ...
        // HID Report characteristic client characteristic configuration
        {
            ...
            GATT_PERMIT_READ | GATT_PERMIT_ENCRYPT_WRITE,
            0,
            (uint8_t *)&hidReportTouchpadInClientCharCfg},
        ...
        // HID Report Reference characteristic descriptor, Touchpad input
        {
            ...
            GATT_PERMIT_READ,
            0,
            hidReportTouchpadRefIn},

    // HID Report characteristic, Touchpad Feature declaration
    {
        {ATT_BT_UUID_SIZE, characterUUID},
        GATT_PERMIT_READ,
        0,
        &hidReportTouchpadFeatureProps},
        ...
        // HID Report characteristic, Touchpad Feature
        {
            ...
            GATT_PERMIT_ENCRYPT_READ | GATT_PERMIT_ENCRYPT_WRITE,
            0,
            hidReportTouchpadFeature},//去掉&

        // HID Report Reference characteristic descriptor, Touchpad Feature
        {
            {ATT_BT_UUID_SIZE, reportRefUUID},
            GATT_PERMIT_READ,
            0,
            hidReportTouchpadFeatureRef},
#endif
#if MOUSE
...
#if TOUCH
    HID_REPORT_TOUCHPAD_IN_DECL_IDX,  // HID Report characteristic, Touchpad input declaration  更新索引用于查表
    HID_REPORT_TOUCHPAD_IN_IDX,       // HID Report characteristic, Touchpad input
    HID_REPORT_TOUCHPAD_IN_CCCD_IDX,  // HID Report characteristic client characteristic configuration
    HID_REPORT_REF_TOUCHPAD_IN_IDX,   // HID Report Reference characteristic descriptor, Touchpad input

    HID_TOUCHPAD_FEATURE_DECL_IDX,        // Touchpad Feature Report declaration
    HID_TOUCHPAD_FEATURE_IDX,             // Touchpad Feature Report
    HID_REPORT_REF_TOUCHPAD_FEATURE_IDX,  // HID Report Reference descriptor, Touchpad Feature
    #endif
#if MOUSE
 ...
 */
bStatus_t Hid_AddService(void)//HID Over GATT Profile 
{
...
#if TOUCH
    GATTServApp_InitCharCfg(INVALID_CONNHANDLE, hidReportTouchpadInClientCharCfg);//初始化cccd使能notify
#endif
#if MOUSE
...
#if TOUCH
    // Touchpad input report 映射表
    hidRptMap[2].id = hidReportTouchpadRefIn[0];
    hidRptMap[2].type = hidReportTouchpadRefIn[1];
    hidRptMap[2].handle = hidAttrTbl[HID_REPORT_TOUCHPAD_IN_IDX].handle;
    hidRptMap[2].cccdHandle = hidAttrTbl[HID_REPORT_TOUCHPAD_IN_CCCD_IDX].handle;
    hidRptMap[2].mode = HID_PROTOCOL_MODE_REPORT;

    // Touchpad Feature report
    hidRptMap[3].id = hidReportTouchpadFeatureRef[0];
    hidRptMap[3].type = hidReportTouchpadFeatureRef[1];
    hidRptMap[3].handle = hidAttrTbl[HID_TOUCHPAD_FEATURE_IDX].handle;
...
//    PRINT("%x \n",hidRptMap[3].cccdHandle);
    ...
    hidRptMap[5].id = hidReportSysCtlRefIn[0];
    hidRptMap[5].type = hidReportSysCtlRefIn[1];
    hidRptMap[5].handle = hidAttrTbl[HID_REPORT_SYS_CTL_IN_IDX].handle;
    hidRptMap[5].cccdHandle = hidAttrTbl[HID_REPORT_SYS_CTL_IN_CCCD_IDX].handle;
    hidRptMap[5].mode = HID_PROTOCOL_MODE_REPORT;
//    PRINT("%x \n",hidRptMap[4].cccdHandle);
#endif

    // ALL_KEY input report
    hidRptMap[6].id = hidReportAllKeyRefIn[0];
    hidRptMap[6].type = hidReportAllKeyRefIn[1];
    hidRptMap[6].handle = hidAttrTbl[HID_REPORT_ALL_KEY_IN_IDX].handle;
    hidRptMap[6].cccdHandle = hidAttrTbl[HID_REPORT_ALL_KEY_IN_CCCD_IDX].handle;
    hidRptMap[6].mode = HID_PROTOCOL_MODE_REPORT;
//    PRINT("%x \n",hidRptMap[5].cccdHandle);

    // FN input report
    hidRptMap[7].id = hidReportFnRefIn[0];
    hidRptMap[7].type = hidReportFnRefIn[1];
    hidRptMap[7].handle = hidAttrTbl[HID_REPORT_FN_IN_IDX].handle;
    hidRptMap[7].cccdHandle = hidAttrTbl[HID_REPORT_FN_IN_CCCD_IDX].handle;
    hidRptMap[7].mode = HID_PROTOCOL_MODE_REPORT;
//    PRINT("%x \n",hidRptMap[7].cccdHandle);

    // SMART_WHEEL input report
    hidRptMap[8].id = hidReportSmartWheelRefIn[0];
    hidRptMap[8].type = hidReportSmartWheelRefIn[1];
    hidRptMap[8].handle = hidAttrTbl[HID_REPORT_SMART_WHEEL_IN_IDX].handle;
    hidRptMap[8].cccdHandle = hidAttrTbl[HID_REPORT_SMART_WHEEL_IN_CCCD_IDX].handle;
    hidRptMap[8].mode = HID_PROTOCOL_MODE_REPORT;
//    PRINT("%x \n",hidRptMap[6].cccdHandle);

    // Boot keyboard input report
    // Use same ID and type as key input report
    hidRptMap[9].id = hidReportRefClassKeyIn[0];
    hidRptMap[9].type = hidReportRefClassKeyIn[1];
    hidRptMap[9].handle = hidAttrTbl[HID_BOOT_KEY_IN_IDX].handle;
    hidRptMap[9].cccdHandle = hidAttrTbl[HID_BOOT_KEY_IN_CCCD_IDX].handle;
    hidRptMap[9].mode = HID_PROTOCOL_MODE_BOOT;
//    PRINT("%x \n",hidRptMap[7].cccdHandle);

    // Boot keyboard output report
    // Use same ID and type as LED output report
    hidRptMap[10].id = hidReportRefLedOut[0];
    hidRptMap[10].type = hidReportRefLedOut[1];
    hidRptMap[10].handle = hidAttrTbl[HID_BOOT_KEY_OUT_IDX].handle;
    hidRptMap[10].cccdHandle = 0;
    hidRptMap[10].mode = HID_PROTOCOL_MODE_BOOT;

    // Feature report
    hidRptMap[11].id = hidReportRefFeature[0];
    hidRptMap[11].type = hidReportRefFeature[1];
    hidRptMap[11].handle = hidAttrTbl[HID_FEATURE_IDX].handle;
    hidRptMap[11].cccdHandle = 0;
